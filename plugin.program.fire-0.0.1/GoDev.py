...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dialogprocess.create("Fire TV","Archiving...",'', 'Please Wait')
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.fire' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def Check_Path():
    if zip=='':
     if dialog.yesno("Fire TV Wizard", "You Have Not Set Your Storage Path", 'Set The Storage Path Now ?',''):
        ADDON.openSettings()
        print '######### ZIP DIRECTORY #########'
...
def RestoreIt():
    ...
    dialog = xbmcgui.Dialog()
    if zip == '':
        dialog.ok('Fire TV Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    dialogprocess.create("Fire TV Wizard","Restoring",'', 'Please Wait')   
    lib=xbmc.translatePath(os.path.join(zip,'backup.zip'))
    Read_Zip(lib)
    dialogprocess.create("Fire TV Wizard","Checking ",'', 'Please Wait')
    ...
    dialogprocess.close()
    dialog.ok("Fire Wizard", "All Done, DONT PRESS OK", "Wait a 5 minutes and pull the Power Cord then","")	
def Backupzip():  
    if zip == '':
        dialog.ok('Fire TV Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    ...
    dialogprocess.create("Fire TV Wizard","Backing Up",'', 'Please Wait')
    ...
    zipobj.close()
    dialogprocess.close()
    dialog.ok("Fire TV Wizard", "You Are Now Backed Up", '','')
