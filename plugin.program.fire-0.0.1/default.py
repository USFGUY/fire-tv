...
def MainMenu():
	...
	if FireTVBuildMenu == 'true':
		addFolder('folder','Fire TV Guru Builds','none', 'FireTVBuildMenu', 'firetvbuilds.png','','','')
	if Maintenance == 'true':
		addFolder('folder','Maintenance and Tools','fanart', 'Tools', 'maintenance.png','','','')	
	setView('movies', 'MAIN')
def FireTVBuildMenu():
    link = OPEN_URL('http://firetvguru.net/build/toolbox.xml').replace('\n','').replace('\r','')
...
def Wipe_Kodi():
    ...
    if choice == 1:
        if skin!= "skin.confluence":
            Dialog.ok('Fire TV Guru','Please Change to the default Confluence skin','before performing a wipe.','')
            ...
            choice = xbmcgui.Dialog().yesno("Remove Fire Wizard?", 'Do you also want to remove the Fire Wizard', 'add-on and have a complete fresh start or would you', 'prefer to keep this on your system?', yeslabel='Remove',nolabel='Keep')
            if choice == 0:
                ...
                DialogProcess.create("Fire TV","Checking ",'', 'Please Wait')
...
def wizard(name,url,description):
    ...
    dp = xbmcgui.DialogProgress()
    dp.create("[COLOR dodgerblue]Fire Wizard Toolbox[/COLOR]","Downloading ",'', 'Please be Patient')
    ...
    Dialog = xbmcgui.Dialog()
    Dialog.ok("Fire TV Wizard", 'Unfortunately the only way to get the new changes to stick is', 'to force close kodi. Click ok to force Kodi to close,', 'DO NOT use the quit/exit options in Kodi.')
    killxbmc()
